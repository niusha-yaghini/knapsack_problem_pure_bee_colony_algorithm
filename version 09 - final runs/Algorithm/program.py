...
for index, row in df.iloc[:].iterrows():
    ...
    # 7-pc, 8-pm(pm/items), 9-selection_type, 10-k_tournoment, 11-crossover_type

    row_values = list(row.iloc)

    run_id, category, problem_num, cpu_time_limit, employed_bees_num, max_try_improve, selection_type, k_tournoment, crossover_type, pc_onePoint, pc_uniForm, pm  = row_values[0], row_values[1], row_values[2], row_values[3], row_values[4], row_values[5], row_values[6], row_values[7], row_values[8], row_values[9], row_values[10], row_values[11]
    ...
    abc = Classic_Artificial_Bee_Colony(run_id, cpu_time_limit, employed_bees_num, nK, nI, Capacity, Profits, Weights, onlooker_bees_num, max_try_improve, pc_onePoint, pc_uniForm, pm, k_tournoment, selection_type, crossover_type, result_file_name)
    ...
    # writing in excel 
    abc.write_excel(run_id, category, problem_num, cpu_time_limit, employed_bees_num, max_try_improve, pc_onePoint, pc_uniForm, pm, selection_type, k_tournoment, crossover_type, best_final_fitness, best_fitness_iteration_num, best_fitness_time, total_iteration_num)
